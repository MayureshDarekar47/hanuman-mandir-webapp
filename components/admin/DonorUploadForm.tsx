...
export default function DonorUploadForm({ onSuccess, children }: Props) {
  ...
  const handleUpload = () => {
    ...
    xhr.open("POST", "/api/upload/donor-file");
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="text-xs text-gray-400 bg-gray-50 rounded-xl px-4 py-3 border border-gray-100">
        {activeTab === "PDF" ? (
          <span>PDF will be stored for download. If it contains parsable text with donor data, records will also be imported automatically.</span>
        ) : (
          <span>
            Column order: <code className="bg-gray-200 px-1 rounded">Name, Amount, Date (dd/mm/yyyy), Note</code> — first row is header and will be skipped.
          </span>
        )}
      </div>
              ...
              <tbody className="divide-y divide-gray-50">
                {preview.map((row, i) => (
                  <tr key={i} className="hover:bg-orange-50/30">
                    ...
                    <td className="py-2 px-4 text-green-700 font-bold">₹{Number(row.amount).toLocaleString("en-IN")}</td>
                    <td className="py-2 px-4 text-gray-500">{row.date ? new Date(row.date).toLocaleDateString("en-IN") : "—"}</td>
                    <td className="py-2 px-4 text-gray-400">{row.note || "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
