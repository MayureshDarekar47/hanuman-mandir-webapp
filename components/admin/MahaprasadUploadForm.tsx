...
export default function MahaprasadUploadForm({ onSuccess, children }: Props) {
  ...
  const handleUpload = () => {
    ...
    xhr.open("POST", "/api/upload/mahaprasad-file");
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="text-xs text-gray-400 bg-gray-50 rounded-xl px-4 py-3 border border-gray-100">
        {activeTab === "PDF" ? (
          <span>PDF will be stored for download. If it contains parsable text with data, records will also be imported automatically.</span>
        ) : (
          <span>
            Column order: <code className="bg-gray-200 px-1 rounded">SrNo, Date, Bhojan Name, Description, StartTime, EndTime</code> — first row is header and will be skipped.
          </span>
        )}
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs text-left">
              <thead className="bg-gray-50 border-b">
                <tr>
                  {["Date", "Name", "Description", "Order Index"].map(h => (
              ...
              <tbody className="divide-y divide-gray-50">
                {preview.map((row, i) => (
                  <tr key={i} className="hover:bg-orange-50/30">
                    ...
                    <td className="py-2 px-4 font-bold">{row.orderIndex}</td>
                  </tr>
                ))}
              </tbody>
            </table>
