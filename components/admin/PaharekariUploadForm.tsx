...
export default function PaharekariUploadForm({ onSuccess, children }: Props) {
  ...
  const handleUpload = () => {
    ...
    xhr.open("POST", "/api/upload/paharekari-file");
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="text-xs text-gray-400 bg-gray-50 rounded-xl px-4 py-3 border border-gray-100">
        {activeTab === "PDF" ? (
          <span>PDF will be stored for download. If it has parsable text with duty data, records will also be imported automatically.</span>
        ) : (
          <span>
            Column order: <code className="bg-gray-200 px-1 rounded">Date (dd/mm/yyyy), Name, Time Start, Time End</code> — first row is header and will be skipped.
          </span>
        )}
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs text-left">
              <thead className="bg-gray-50 border-b">
                <tr>
                  {["Date", "Name", "Time Start", "Time End"].map(h => (
              ...
              <tbody className="divide-y divide-gray-50">
                {preview.map((row, i) => (
                  <tr key={i} className="hover:bg-orange-50/30">
                    ...
                    <td className="py-2 px-4 text-gray-500">{row.startTime || "—"}</td>
                    <td className="py-2 px-4 text-gray-500">{row.endTime || "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
