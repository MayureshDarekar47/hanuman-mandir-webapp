...
// Helper: parse Excel buffer into donor records
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (fileName.endsWith(".pdf") || file.type === "application/pdf") {
      ...
      const storagePath = `donors/${Date.now()}_${safeName}`;
      ...
      let parsedRecords: { name: string; amount: number; date: Date; note?: string }[] = [];
      let pdfText = "";
      try {
      ...
      if (parsedRecords.length > 0) {
        await prisma.donor.createMany({ data: parsedRecords });
        ...
        return NextResponse.json({
          ...
          message: `Parsed ${parsedRecords.length} donor records from PDF. File also stored for download.`,
        });
      }
      ...
      return NextResponse.json({
        ...
        count: 0,
        downloadUrl: urlData.publicUrl,
        message: "PDF stored for download. Could not auto-parse donor records (scanned/image PDF).",
      });
    }
    ...
    if (
      ...
      return NextResponse.json({
        ...
        message: `Successfully imported ${records.length} donor records from Excel.`,
    ...
    if (
      ...
      return NextResponse.json({
        ...
        type: "csv",
        count: records.length,
        message: `Successfully imported ${records.length} donor records from CSV.`,
        preview: records.slice(0, 5),
      });
    }
    ...
    console.error("[donor-file upload]", err);
